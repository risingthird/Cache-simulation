...
void setBit(cache_t* cache, uint64_t location, uint8_t value) {
    ...
    /*if(shiftAmount == 0){
        ...
        if(value==0)
            cache->contents[byteLoc] = cache->contents[byteLoc] & mask;
        else
            cache->contents[byteLoc] = cache->contents[byteLoc] | (value<<7);
    ...
    uint8_t temp = cache->contents[byteLoc];
    temp = temp &(~(1<<(7-shiftAmount))) | (value<<(7-shiftAmount));
    cache->contents[byteLoc] = temp;
...
void setLRU(cache_t* cache, uint32_t blockNumber, long newLRU) {
    ...
    int shiftAmount = location & 7;
    if(shiftAmount == 0)
        cache->contents[byteLoc] = (uint8_t)newLRU;
    else{
